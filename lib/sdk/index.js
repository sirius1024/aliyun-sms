...
class SMSClient {
    ...
    async receiveMsg(typeIndex = 0, preQueueName, waitSeconds = 10) {
        ...
        let result = {}
        try {
            let received = await mnsClient.receiveMessage(preQueueName + msgTypeList[typeIndex], waitSeconds);
            result.receiveMsg = received;
            let deleteInfo = await mnsClient.deleteMessage(preQueueName + msgTypeList[typeIndex], received.body.ReceiptHandle);
            result.deleteMsg = deleteInfo;
            return result;
        } catch (exp) {
            console.log(result.receiveMsg);
            throw exp;
        }
    }
}
